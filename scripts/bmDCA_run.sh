...
SAVE_PARAMETERS=20   # numero di iterazioni ogni quale si salvano i parametri
...
ERROR_MIN_UPDATE=-1 # minimal number of standard deviations of z varible for
                    # having parameter update (if negative or zero all
...
CHECK_ERGO=1          # if 1, routine to check if MC is well thermalized and
                      # decorrelated is active
...
then
  ...
  initialize $N $Q learning_rate.txt $EPSILON_0_J $EPSILON_0_h
else
...
do
  ...
  do
    ...
    echo 3b-check correct thermalization and decorrelation
  ...
  echo 3c importance sampling loop
  step_importance=0
  flag_coherence=1
  ...
  do
    ...
    echo 3f-compare errors and save
    ...
    ##### 3h save parameters
    echo 3h save parameters
    resto=$(( $step % $SAVE_PARAMETERS ))
    if [ $resto == 0 ]
    then
