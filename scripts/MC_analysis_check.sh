...
then
  ...
  MCMC_rip -n $N -q $Q -m $M_ERG -T $T_WAIT_ERG -t $DELTA_T_ERG -s 0 -r $COUNT_MAX_ERG < parameters_temp.txt
  tail -n +2 out_samples_montecarlo*.txt >> $MC_file_ERG
  rm out_*.txt
fi

autocorrelation $MC_file_ERG $COUNT_MAX_ERG $DELTA_T_ERG overlap.txt
plot_overlap.sh
...
then
  ...
  MCMC_rip -n $N -q $Q -m $M_REL -T $T_WAIT_REL -t $DELTA_T_REL -s 0 -r $COUNT_MAX_REL < parameters_temp.txt
  tail -n +2 out_samples_montecarlo*.txt >> $MC_file_REL
  rm out_*.txt
fi

compute_relaxation $MC_file_REL parameters_temp.txt $COUNT_MAX_REL $DELTA_T_REL energy.dat
plot_energy.sh
...
# compute_energies $Test_file parameters_temp.txt energy_temp_erg.dat
# compute_energies $Test_file $PARAMETERS_TRUE_FILE energy_true_erg.dat
# compare_energies energy_temp_erg.dat energy_true_erg.dat $MNEW_ERG energy_cfr_erg.txt
