...
#define EPSILON                                                                \
  0.00000001 // regularization parameter to estimate deviations (should be
...
main(int argc, char* argv[])
{
  ...
  double* n2mc = (double*)malloc(sizeof(double) * q * q * N * N);
  double* n1mcsigma = (double*)malloc(sizeof(double) * q * N);
  double* n2mcsigma = (double*)malloc(sizeof(double) * q * q * N * N);
  double* gradh = (double*)malloc(sizeof(double) * q * N);
  double* gradJ = (double*)malloc(sizeof(double) * q * q * N * N);
  double* h = (double*)malloc(sizeof(double) * q * N);
  double* J = (double*)malloc(sizeof(double) * q * q * N * N);
  ...
  for (i = 0; i < N; i++) {
    ...
    for (a = 0; a < q; a++) {
      ...
      if (sqrt(pow(delta_stat, 2)) > ERROR_MIN_UPDATE) {
        gradh[a + q * i] =
          n1[a + q * i] - n1mc[a + q * i] - LAMBDA_h * h[a + q * i];
        count1++;
      }
      fprintf(fperror,
              "%d %d %lf %lf %lf %lf %lf\n",
    ...
    for (j = i + 1; j < N; j++) {
      for (a = 0; a < q * q; a++) {
        ...
        delta_stat =
          (n2mc[a + q * q * j + i * N * q * q] -
           n2[a + q * q * j + i * N * q * q]) /
          (pow(n2[a + q * q * j + i * N * q * q] *
                   (1.0 - n2[a + q * q * j + i * N * q * q]) / MEFF +
                 pow(n2mcsigma[a + q * q * j + i * N * q * q], 2) + EPSILON,
               0.5));

        c_mc = n2mc[a + q * q * j + i * N * q * q] -
               n1mc[a1 + q * i] * n1mc[a2 + q * j];
        c_stat =
          n2[a + q * q * j + i * N * q * q] - n1[a1 + q * i] * n1[a2 + q * j];
        c_mc_av += c_mc;
        c_stat_av += c_stat;
