...
27. `use_pos_reg` - flag to apply position-specific regularization when
    learning J (default false)
28. `temperature` - temperature at which to sample sequences (default 1)
29. `t_wait_check` - _unused_ burn-in time for when running check MCMC (default
    10000)
30. `delta_t_check` - _unused_ wait time between sampled sequences when running
    MCMC check (default 100)
31. `M_check` - number of sequences to sample for each MCMC check replicate
    (default 1000)
32. `count_check` - number of replicates for the MCMC check (default 10)
